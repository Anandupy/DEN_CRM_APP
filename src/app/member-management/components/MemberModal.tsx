...
import { Member, PlanPrice, Plan, Gender, PaymentMode } from './memberData';

interface MemberModalProps {
  member: Member | null;
  trainerOptions: string[];
  planOptions: PlanPrice[];
...
function getExpiryFromPlan(joinDate: string, plan: Plan, planOptions: PlanPrice[]): string {
  const d = new Date(joinDate);
  const selectedPlan = planOptions.find((option) => option.plan === plan);
  const durationMonths = Number.parseInt(selectedPlan?.duration ?? '1', 10) || 1;
  d.setMonth(d.getMonth() + durationMonths);
  return d.toISOString().split('T')[0];
}

export default function MemberModal({ member, trainerOptions, planOptions, onSave, onClose }: MemberModalProps) {
  const isEditing = !!member;
  const [isLoading, setIsLoading] = React.useState(false);
  const [activeSection, setActiveSection] = React.useState<'personal' | 'membership' | 'medical'>('personal');
  const availableTrainers = trainerOptions.length ? trainerOptions : ['Unassigned'];
  const availablePlans = planOptions.length ? planOptions : [
  ...
  useEffect(() => {
    if (watchedJoinDate && watchedPlan) {
      setValue('expiryDate', getExpiryFromPlan(watchedJoinDate, watchedPlan as Plan, availablePlans));
    }
  }, [watchedPlan, watchedJoinDate, setValue, availablePlans]);

  useEffect(() => {
    if (member) return;
    if (availablePlans[0] && watchedPlan !== availablePlans[0].plan) {
      setValue('plan', availablePlans[0].plan);
    }
    if (availableTrainers[0] && watchedTrainer !== availableTrainers[0]) {
      setValue('trainer', availableTrainers[0]);
    }
  }, [member, availablePlans, availableTrainers, setValue, watchedPlan, watchedTrainer]);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="form-input"
                    >
                        {availablePlans.map((plan) => <option key={`plan-sel-${plan.plan}`} value={plan.plan}>{plan.plan}</option>)}
                    </select>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        className="form-input pl-9"
                      >
                        {availableTrainers.map((trainer) => <option key={`trainer-sel-${trainer}`} value={trainer}>{trainer}</option>)}
                      </select>
                    </div>
