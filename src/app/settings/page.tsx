...
import { loadPlanMasterData, savePlanPricingInSupabase } from '@/lib/erp/data';
...
export default function SettingsPage() {
  ...
  const [plans, setPlans] = useState([
    { plan: 'Monthly', price: 1500, durationMonths: 1, durationLabel: '1 Month', isActive: true },
    { plan: 'Quarterly', price: 3500, durationMonths: 3, durationLabel: '3 Months', isActive: true },
    { plan: 'Half-Yearly', price: 6500, durationMonths: 6, durationLabel: '6 Months', isActive: true },
    { plan: 'Annual', price: 12000, durationMonths: 12, durationLabel: '12 Months', isActive: true },
  ]);
  const [plansSource, setPlansSource] = useState<'supabase' | 'fallback'>('fallback');
  ...
  useEffect(() => {
    loadPlanMasterData().then((rows) => {
      setPlans(rows);
      setPlansSource(isSupabaseConfigured() ? 'supabase' : 'fallback');
    });
  }, []);

  const handleSave = async () => {
    try {
      if (activeTab === 'plans' && isSupabaseConfigured()) {
        await savePlanPricingInSupabase(plans);
        setPlansSource('supabase');
      }
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
      toast.success(activeTab === 'plans' ? 'Membership plans updated' : 'Settings saved');
    } catch (error) {
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-[11px] uppercase tracking-wide text-amber-500/80">
                    {plansSource === 'supabase' ? 'Live master data' : 'Fallback data'}
                  </span>
                </div>
                {plans.map((plan, i) => (
                  <div key={plan.plan} className="p-4 bg-zinc-800/50 border border-zinc-700/50 rounded-xl">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          value={plan.plan}
                          readOnly
                          className="form-input text-xs opacity-70 cursor-not-allowed"
                        />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={e => setPlans(prev => prev.map((p, idx) => idx === i ? {
                            ...p,
                            durationMonths: Number(e.target.value),
                            durationLabel: `${e.target.value} ${Number(e.target.value) === 1 ? 'Month' : 'Months'}`,
                          } : p))}
                          className="form-input text-xs"
                        />
                      </div>
                      <div className="sm:col-span-3 flex items-center justify-between rounded-lg border border-zinc-700/50 bg-zinc-900/40 px-3 py-2">
                        <div>
                          <p className="text-xs text-zinc-400">Duration Label</p>
                          <p className="text-sm text-zinc-200">{plan.durationLabel}</p>
                        </div>
                        <label className="flex items-center gap-2 text-xs text-zinc-300">
                          <input
                            type="checkbox"
                            checked={plan.isActive}
                            onChange={e => setPlans(prev => prev.map((p, idx) => idx === i ? { ...p, isActive: e.target.checked } : p))}
