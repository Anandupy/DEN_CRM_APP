...
export default function TrainerDashboardPage() {
  ...
  const [members, setMembers] = useState<Array<{
    ...
    attendance: null | 'present' | 'absent' | 'late';
    phone: string;
    joinDate: string;
  }>>([]);
  ...
  useEffect(() => {
    ...
    loadErpSnapshot().then(async (snapshot) => {
      const attendanceMap = isSupabaseConfigured()
        ? await loadAttendanceStatusMap(today)
        : Object.fromEntries(snapshot.attendance.map((entry) => [entry.memberId, entry.status]));

      const assigned = snapshot.members
        .filter((member) => member.trainer === profile?.full_name)
        .map((member) => ({
          id: member.memberId,
          name: member.name,
  ...
  const markAttendance = async (id: string, status: 'present' | 'absent' | 'late') => {
    try {
  ...
  return (
    <AppLayout activePath="/trainer-dashboard">
      <div className="space-y-6 fade-in">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-zinc-100">Good Morning, {profile?.full_name?.split(' ')[0] ?? 'Trainer'} 👋</h1>
